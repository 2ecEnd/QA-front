    ...
    async render() {
      ...
      <div class="filters-panel">
        ...
        <select id="filter-category" class="filter-select">
          ...
          ${Object.entries(categoryLabels).filter(([k]) => ['DESSERT', 'FIRST_COURSE', 'SECOND_COURSE', 'DRINK', 'SALAD', 'SOUP', 'SNACK'].includes(k)).map(([val, label]) => `<option value="${val}">${label}</option>`).join('')}
        </select>
