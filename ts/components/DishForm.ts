...
export class DishFormPage {
          ...
          <div class="form-row">
            <div class="form-group">
              ...
              <select id="category">
                ...
                ${['DESSERT','FIRST_COURSE','SECOND_COURSE','DRINK','SALAD','SOUP','SNACK'].map(cat => 
                  `<option value="${cat}" ${initialData.category === cat ? 'selected' : ''}>${categoryLabels[cat]}</option>`
                ).join('')}
              </select>
            </div>
          </div>
  ...
  private recalculateNutrition() {
    ...
    (document.getElementById('carbohydrates') as HTMLInputElement).value = totalCarbs.toFixed(1);
    (document.getElementById('portionSize') as HTMLInputElement).value = totalWeight.toFixed(1);
  }

  private updateFlagsAvailability() {
    if (this.ingredients.length === 0) {
    ...
    for (const ing of this.ingredients) {
      const product = this.availableProducts.find(p => p.id === ing.productId);
      if (!product) continue;
      if (!product.flags.includes('VEGAN')) veganAvailable = false;
      if (!product.flags.includes('GLUTEN_FREE')) glutenFreeAvailable = false;
      if (!product.flags.includes('SUGAR_FREE')) sugarFreeAvailable = false;
    }
    ...
    if (!veganAvailable) veganCheck.checked = false;
    if (!glutenFreeAvailable) glutenCheck.checked = false;
    if (!sugarFreeAvailable) sugarCheck.checked = false;
  }

  private setFlagsEnabled(vegan: boolean, gluten: boolean, sugar: boolean) {
    (document.getElementById('flag-vegan') as HTMLInputElement).disabled = !vegan;
    (document.getElementById('flag-gluten') as HTMLInputElement).disabled = !gluten;
    (document.getElementById('flag-sugar') as HTMLInputElement).disabled = !sugar;
  }
  ...
  private async handleSubmit(e: Event) {
    ...
    const data: DishCreateDto = {
      ...
      proteins: parseFloat(formData.get('proteins') as string) || undefined,
      fats: parseFloat(formData.get('fats') as string) || undefined,
      carbohydrates: parseFloat(formData.get('carbohydrates') as string) || undefined,
      portionSize: parseFloat(formData.get('portionSize') as string),
      category,
      flags,
